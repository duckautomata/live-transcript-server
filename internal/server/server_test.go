...
func TestServer_ActivateDeactivate(t *testing.T) {
	...
	// Verify DB state
	stream, err := app.Store.GetRecentStream(ctx, key)
	if err != nil {
	...
	if err := app.Store.ReplaceTranscript(ctx, key, "stream1", lines); err != nil {
		t.Fatalf("failed to replace transcript: %v", err)
	}
	// Verify transcripts are there
	transcripts, err := app.Store.GetTranscript(ctx, key, "stream1")
	if err != nil {
	...
	transcripts, err = app.Store.GetTranscript(ctx, key, "stream2")
	if err != nil {
		t.Fatalf("failed to get transcripts: %v", err)
	}
...
func TestServer_LineUpdate(t *testing.T) {
	...
	// Verify DB
	lines, err := app.Store.GetTranscript(context.TODO(), key, "stream2")
	if err != nil {
...
func TestServer_MediaUpload(t *testing.T) {
	...
	stream := &model.Stream{ChannelID: key, StreamID: "stream_media", StreamTitle: "Test Stream", StartTime: "12345", IsLive: true, MediaType: "audio"}
	...
	// Inject a fake media processor instead of shelling out to ffmpeg.
	app.Media = fakeProcessor{}

	// Check if file exists using FileID from DB
	transcript, err := app.Store.GetTranscript(ctx, key, "stream_media")
	if err != nil {
		t.Fatalf("failed to get transcript: %v", err)
	}
	if len(transcript) != 0 {
	...
	app.Store.InsertNextLine(ctx, key, "stream_media", model.Line{ID: 0, Timestamp: 100})
	...
	lines, err := app.Store.GetTranscript(ctx, key, "stream_media")
	if err != nil {
		t.Fatalf("failed to get transcript: %v", err)
	}
	if len(lines) != 1 {
...
func TestServer_Sync(t *testing.T) {
	...
	app.Store.UpsertStream(context.TODO(), &model.Stream{ChannelID: key, StreamID: "stream3", StreamTitle: "Test Sync", StartTime: fmt.Sprintf("%d", time.Now().Unix()), IsLive: true, MediaType: "none"})
	app.Store.InsertNextLine(context.TODO(), key, "stream3", model.Line{ID: 0, Timestamp: 100})
	...
	// Verify DB state
	stream, err := app.Store.GetRecentStream(context.TODO(), key)
	if err != nil {
		t.Fatalf("failed to get stream: %v", err)
	}
	...
	lines, err := app.Store.GetTranscript(context.TODO(), key, "stream3")
	if err != nil {
		t.Fatalf("failed to get transcript: %v", err)
	}
	if len(lines) != 2 {
	...
	if err := app.Store.SetMediaAvailable(context.TODO(), key, "stream3", 1, "1", true); err != nil {
		t.Fatalf("failed to set media available: %v", err)
	}

	// 2. Sync again with MediaAvailable set to false for line 1
	syncData2 := ws.EventSyncData{
		StreamID:    "stream3",
	...
	if err := app.Store.InsertNextLine(context.TODO(), key, "stream3", model.Line{ID: 2, Timestamp: 300}); err != nil {
		t.Fatalf("failed to insert line 2: %v", err)
	}
	if err := app.Store.SetMediaAvailable(context.TODO(), key, "stream3", 2, "2", true); err != nil {
		t.Fatalf("failed to set media available: %v", err)
	}

	// Sync with line 2 having MediaAvailable false
	syncData3 := ws.EventSyncData{
		StreamID:    "stream3",
...
func TestServer_Persistence(t *testing.T) {
	...
	st, err := store.Open(dbPath, config.DatabaseConfig{SkipWarmup: true})
	if err != nil {
	...
	testConfig := config.Config{
		Credentials: config.Credentials{ApiKey: apiKey},
		Channels:    []config.ChannelConfig{{Name: key, NumPastStreams: 1}},
		Storage:     config.StorageConfig{Type: "local"},
	}
	app, err := NewApp(testConfig, st, dir, "test-version", "test-build-time")
	if err != nil {
	...
	// Re-open DB
	st2, err := store.Open(dbPath, config.DatabaseConfig{SkipWarmup: true})
	if err != nil {
	...
	stream, err := st2.GetRecentStream(context.TODO(), key)
	if err != nil {
		t.Fatalf("failed to get stream: %v", err)
	}
	if stream == nil {
...
func TestServer_GetTranscriptEndpoint(t *testing.T) {
	...
	// Seed data
	stream1ID := "stream1"
	app.Store.UpsertStream(ctx, &model.Stream{ChannelID: key, StreamID: stream1ID, IsLive: false})
	app.Store.ReplaceTranscript(ctx, key, stream1ID, []model.Line{
		{ID: 0, Segments: json.RawMessage(`[{"text": "Stream 1 Line 0"}]`)},
	})

	stream2ID := "stream2"
	app.Store.UpsertStream(ctx, &model.Stream{ChannelID: key, StreamID: stream2ID, IsLive: true})
	app.Store.ReplaceTranscript(ctx, key, stream2ID, []model.Line{
	...
	// Should return empty list (or 200 with empty list), logic returns []model.Line{} which is valid JSON "[]"
	if rr.Code != http.StatusOK {
...
func TestServer_MediaEndpoints(t *testing.T) {
	...
	// 1. Setup Data
	// Activate stream
	app.Store.UpsertStream(ctx, &model.Stream{ChannelID: key, StreamID: "s1", StreamTitle: "Stream 1", StartTime: "12345", IsLive: true, MediaType: "video"})
	...
	for i := 0; i <= 10; i++ {
		...
		app.Store.InsertNextLine(ctx, key, "s1", line)
	...
	// Fake media processor for clip/trim (no real ffmpeg).
	app.Media = fakeProcessor{
		convert: func(in, out string) error { return os.WriteFile(out, []byte("converted"), 0644) },
		trim:    func(in, out string, start, end float64) error { return os.WriteFile(out, []byte("trimmed"), 0644) },
		remux:   func(in, out string) error { return os.WriteFile(out, []byte("remuxed"), 0644) },
	}
...
func TestServer_ActivateStream_Retention_UnderThreshold(t *testing.T) {
	...
	p1 := &model.Stream{ChannelID: key, StreamID: "p1", StartTime: "2000", IsLive: false}
	app.Store.UpsertStream(ctx, p1)
	app.Store.InsertNextLine(ctx, key, "p1", model.Line{ID: 0, Segments: json.RawMessage(`[{"text": "P1 Content"}]`)})
	...
	// Verify S1 is active
	s, err := app.Store.GetRecentStream(ctx, key)
	if err != nil {
	...
	past, err := app.Store.GetPastStreams(ctx, key, "s1")
	if err != nil {
	...
	lines, _ := app.Store.GetTranscript(ctx, key, "p1")
	if len(lines) == 0 {
...
func TestServer_ActivateStream_Retention_EqualThreshold(t *testing.T) {
	...
	// Initialize S1 (Live) and P1 (Past)
	p1 := &model.Stream{ChannelID: key, StreamID: "p1", StartTime: "2000", IsLive: false, ActivatedTime: 2000}
	s1 := &model.Stream{ChannelID: key, StreamID: "s1", StartTime: "3000", IsLive: true, ActivatedTime: 3000}
	app.Store.UpsertStream(ctx, p1)
	app.Store.UpsertStream(ctx, s1)
	app.Store.InsertNextLine(ctx, key, "p1", model.Line{ID: 0, Segments: json.RawMessage(`[{"text": "P1 Content"}]`)})
	...
	past, err := app.Store.GetPastStreams(ctx, key, "s2")
	if err != nil {
		t.Fatalf("GetPastStreams failed: %v", err)
	}
	if len(past) != 2 {
...
func TestServer_ActivateStream_Retention_OverThreshold(t *testing.T) {
	...
	// Setup: S2 (Active), S1, P1
	p1 := &model.Stream{ChannelID: key, StreamID: "p1", StartTime: "2000", IsLive: false, ActivatedTime: 2000}
	s1 := &model.Stream{ChannelID: key, StreamID: "s1", StartTime: "3000", IsLive: false, ActivatedTime: 3000}
	s2 := &model.Stream{ChannelID: key, StreamID: "s2", StartTime: "4000", IsLive: true, ActivatedTime: 4000}
	app.Store.UpsertStream(ctx, p1)
	app.Store.UpsertStream(ctx, s1)
	app.Store.UpsertStream(ctx, s2)
	...
	past, err := app.Store.GetPastStreams(ctx, key, "s3")
	if err != nil {
		t.Fatalf("GetPastStreams failed: %v", err)
	}
	if len(past) != 2 {
...
func TestServer_ActivateStream_Retention_MassiveOverflow(t *testing.T) {
	...
	// Setup: S3 (Active), S2, S1, P4, P5
	p5 := &model.Stream{ChannelID: key, StreamID: "p5", StartTime: "0100", IsLive: false, ActivatedTime: 100} // Very old
	p4 := &model.Stream{ChannelID: key, StreamID: "p4", StartTime: "0500", IsLive: false, ActivatedTime: 500} // Old
	s1 := &model.Stream{ChannelID: key, StreamID: "s1", StartTime: "3000", IsLive: false, ActivatedTime: 3000}
	s2 := &model.Stream{ChannelID: key, StreamID: "s2", StartTime: "4000", IsLive: false, ActivatedTime: 4000}
	s3 := &model.Stream{ChannelID: key, StreamID: "s3", StartTime: "5000", IsLive: true, ActivatedTime: 5000}

	app.Store.UpsertStream(ctx, p5)
	app.Store.UpsertStream(ctx, p4)
	app.Store.UpsertStream(ctx, s1)
	app.Store.UpsertStream(ctx, s2)
	app.Store.UpsertStream(ctx, s3)

	app.Store.InsertNextLine(ctx, key, "s1", model.Line{ID: 0, Segments: json.RawMessage(`[{"text": "S1 Content"}]`)})
	...
	past, err := app.Store.GetPastStreams(ctx, key, "s4")
	if err != nil {
		t.Fatalf("GetPastStreams failed: %v", err)
	}
	if len(past) != 2 {
	...
	// Verify S1 deleted
	lines, _ := app.Store.GetTranscript(ctx, key, "s1")
	if len(lines) != 0 {
...
func TestNewApp_WorkerStatusInitialization(t *testing.T) {
	...
	st, err := store.Open(dbPath, config.DatabaseConfig{})
	if err != nil {
		t.Fatalf("Failed to init db: %v", err)
	}
	...
	// Insert a stale record to verify Init resets the table
	staleKey := "stale-key"
	if err := st.UpsertWorkerStatus(ctx, staleKey, "old", "old", 0); err != nil {
	...
	app, err := NewApp(testConfig, st, dir, version, buildTime)
	if err != nil {
		t.Fatalf("Failed to construct app: %v", err)
	}
	...
	// Check worker_status
	statuses, err := app.Store.GetAllWorkerStatus(ctx)
	if err != nil {
...
func TestPostClipHandler_StreamID(t *testing.T) {
	...
	stream1 := &model.Stream{
		ChannelID: key,
		StreamID:  "s1",
		IsLive:    false,
		MediaType: "video",
	}
	app.Store.UpsertStream(ctx, stream1)
	...
	app.Store.UpsertStream(ctx, stream2)
